...
class Penilaian extends CI_Controller {
	...
	public function legerKd()
	{
		...
		$data['kd_perkelas'] = $this->um->legerKd($filter_kelas);
	...
	public function cetakmii($id,$id_kelas)
	{
		...
		$na = $this->um->rapotMii($tahun_id,$id);
		$filter_kelas = $this->cekwali()['kelas_id'];

		$data['at_wali'] = $this->um->showNamaWali($filter_kelas)['nama_asatid'];
		...
		foreach ($na as $na) {
			$nilai[$na['mapel_id']] = [
					...
					'rata2' => round($this->um->rataNrp($id_kelas, $na['mapel_id']),0)
			];
		...
		$jumlah['rata2'] = $jumlah['nrp'] / $banyak; 
