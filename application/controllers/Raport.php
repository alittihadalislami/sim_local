...
class Raport extends CI_Controller {
	...
	public function pdfma($santri,$kls)
	{
		...
		// $this->load->view('raport/raport_ma_pdf',$data);
...
/*		$mapel = 17;
		...
		$deskrip ['min'] = [
			...
			'desk' => strtolower($this->rm->pilihDeskripsiKD($tahun, $mapel, $kelas, $min_terpilih, $jenis_kd)["kd$jenis_kd"]),
		];
		$deskrip ['max'] = [
			'pre'=>$this->hitungPredikat(max($kd),75)[1],
			'desk'=>strtolower($this->rm->pilihDeskripsiKD($tahun, $mapel, $kelas, $max_terpilih, $jenis_kd)["kd$jenis_kd"]),
		];
	...
	public function konfersi($kelas)
	{
		...
		foreach ($mapel as $mp) {
			...
			$kkm = $this->rm->kkm($kelas, $mp['mapel_id'], $this->tahunAktif['id_tahun']);
			...
			if ($kete_terkecil < $kkm) { //jika nilai Keterampilan kurang dari kkm
				...
				foreach ($target_siswa as $ts) { //ekeskusi setiap siswa
					$id = $ts['id_dkn_raport'];
					$nilai_p = $ts['k'];
					$nilai_p_kantrol = $kkm + (( $ts['k'] - $kete_terkecil)/($kete_terbesar-$kete_terkecil)*($target_max_k-$kkm)) ;
					$data = ['k'=>round($nilai_p_kantrol,0)];
					$this->db->update('t_dkn_raport', $data, ['id_dkn_raport'=>$id]);
				}
			}
