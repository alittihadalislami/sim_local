...
class Raport_model extends CI_Model {
	...
	public function kkm($kelas, $mapel, $tahun)
	{
		$stringQ=" SELECT d.`kelas_id`, d.`mapel_id`, d.`kkm`
					FROM t_kd AS d
					WHERE d.`kelas_id`=$kelas AND d.`mapel_id` = $mapel
	...
	public function pilihDeskripsiKD($tahun, $mapel, $kelas, $urut, $kd='p')
	{
		if ($kd == 'p') {
			$kolom = 'p';
		}else{
		...
		$stringQ="SELECT d.`urut`, d.`kd$kolom`
				FROM t_kd AS d
				WHERE d.`tahun_id`=$tahun AND d.`kelas_id`=$kelas AND d.`mapel_id`=$mapel AND d.`urut`=$urut";
		return $this->db->query($stringQ)->row_array();
	}
