...
class User_model extends CI_Model {
	...
	public function adaIdWali($id_asatid)
	{
		return $this->db->get_where('t_wali', ["asatid_id" => $id_asatid ])->row_array();
	}

	public function legerKd($kelas_id){
		$this->db->order_by('mapel_id', 'asc');
		return $this->db->get_where('t_kd', ['kelas_id' => $kelas_id])->result_array();
	}
	...
	public function showNamaWali($id_kelas){
		$this->db->select('asatid_id');
		$asatid_id = $this->db->get_where('t_wali', ['kelas_id' => $id_kelas])->row_array();
	...
	public function rapotMii($id_tahun, $id_santri){
		$stringQ = " SELECT a.*, m.`nama_mapel`, m.`mapel_alias`, m.`mapel_ar`
					...
					WHERE a.`tahun_id`=$id_tahun AND a.`santri_id` =  $id_santri
