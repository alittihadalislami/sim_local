		...
		<table table-layout= "fixed">
			...
			<tbody>
    			...
    			<tr>
    				...
    				<?php $kkm = $this->rm->kkm($id_kelas, 31, $this->tahunAktif['id_tahun']) ?>
    				...
    				<td class="teks"><?= $CI->tampilPredikat($id_santri, $id_kelas, 31, 'p') ?></td>
    
    				<td><?= $dkn[$id_santri]['nilai'][31]['k'] ?></td>
    				<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][31]['k'], $kkm)[0] ?></td>
    				<td class="teks"><?= $CI->tampilPredikat($id_santri, $id_kelas, 31, 'k') ?></td>
    			</tr>
    			<tr>
    			    <?php $kkm = $this->rm->kkm($id_kelas, 40, $this->tahunAktif['id_tahun']) ?>
    				<td></td>
    				<td>b. Aqidah Akhlaq <br><p class="kkm">KKM:75</p></td>
    				<td><?= $dkn[$id_santri]['nilai'][40]['p'] ?></td>
    				<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][40]['p'], 75)[0] ?></td>
    				<td class="teks"><?= $CI->tampilPredikat($id_santri, $id_kelas, 40, 'p') ?></td>
    
    				<td><?= $dkn[$id_santri]['nilai'][40]['k'] ?></td>
    				<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][40]['k'], 75)[0] ?></td>
    				<td class="teks"><?= $CI->tampilPredikat($id_santri, $id_kelas, 40, 'k') ?></td>
    			</tr>
    			<tr>
    			    <?php $kkm = $this->rm->kkm($id_kelas, 8, $this->tahunAktif['id_tahun']) ?>
    				<td></td>
    				<td>c. Fiqh<br><p class="kkm">KKM:<?=$kkm ?></p></td>
    				<td><?= $dkn[$id_santri]['nilai'][8]['p'] ?></td>
    				<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][8]['p'], 75)[0] ?></td>
    				<td class="teks"><?= $CI->tampilPredikat($id_santri, $id_kelas, 8, 'p') ?></td>
    
    				<td><?= $dkn[$id_santri]['nilai'][8]['k'] ?></td>
    				<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][8]['k'], 75)[0] ?></td>
    				<td class="teks"><?= $CI->tampilPredikat($id_santri, $id_kelas, 8, 'k') ?></td>
    			</tr>
    			<tr>
    			    <?php $kkm = $this->rm->kkm($id_kelas, 33, $this->tahunAktif['id_tahun']) ?>
    				<td></td>
    				<td>d. Sejarah Kebudayaan Islam<br><p class="kkm">KKM:<?=$kkm ?></p></td>
    				<td><?= $dkn[$id_santri]['nilai'][33]['p'] ?></td>
    				<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][33]['p'], 75)[0] ?></td>
    				<td class="teks"><?= $CI->tampilPredikat($id_santri, $id_kelas, 33, 'p') ?></td>
    
    				<td><?= $dkn[$id_santri]['nilai'][33]['k'] ?></td>
    				<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][33]['k'], 75)[0] ?></td>
    				<td class="teks"><?= $CI->tampilPredikat($id_santri, $id_kelas, 33, 'k') ?></td>
    			</tr>
			    <?php $no=2; foreach ($kel1 as $k1): ?>
				<?php $kkm = $this->rm->kkm($id_kelas, $k1, $this->tahunAktif['id_tahun']) ?>
				<tr>
					<td><?=$no++?></td>
					...
					<td class="teks"><?= $CI->tampilPredikat($id_santri, $id_kelas, $k1, 'p') ?></td>

					<td><?= $dkn[$id_santri]['nilai'][$k1]['k'] ?></td>
					<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][$k1]['k'], $kkm)[0] ?></td>
					<td class="teks"><?= $CI->tampilPredikat($id_santri, $id_kelas, $k1, 'k') ?></td>
				</tr>
			    <?php endforeach ?>
    			<tr>
			    ...
			    <?php $kkm = $this->rm->kkm($id_kelas, $k2, $this->tahunAktif['id_tahun']) ?>
				<tr>
					<td><?= $no++ ?></td>
					<td><?= $dkn[$id_santri]['nilai'][$k2]['nama_mapel'] ?><br><p class="kkm">KKM:<?=$kkm ?></p></td>
					<td><?= $dkn[$id_santri]['nilai'][$k2]['p'] ?></td>
					<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][$k2]['p'], 75)[0] ?></td>
					<td class="teks isi-deskrip"><?= $CI->tampilPredikat($id_santri, $id_kelas, $k2, 'p') ?></td>

					<td><?= $dkn[$id_santri]['nilai'][$k2]['k'] ?></td>
					<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][$k2]['k'], 75)[0] ?></td>
					<td class="teks isi-deskrip"><?= $CI->tampilPredikat($id_santri, $id_kelas, $k2, 'k') ?></td>
				</tr>
			    <?php endforeach ?>
    			<tr>
    			    <?php $kkm = $this->rm->kkm($id_kelas, 22, $this->tahunAktif['id_tahun']) ?>
    				<td>4</td>
    				<td>Keterampilan<br><p class="kkm">KKM:<?=$kkm ?></p></td>
    				<td><?= $dkn[$id_santri]['nilai'][22]['p'] ?></td>
    				<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][22]['p'], 75)[0] ?></td>
    				<td class="teks isi-deskrip"><?= $CI->tampilPredikat($id_santri, $id_kelas, 22, 'p') ?></td>
    
    				<td><?= $dkn[$id_santri]['nilai'][22]['k'] ?></td>
    				<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][22]['k'], 75)[0] ?></td>
    				<td class="teks isi-deskrip"><?= $CI->tampilPredikat($id_santri, $id_kelas, 22, 'k') ?></td>
    			</tr>
    			<tr>
    				<td colspan="8">Kelompok C (Peminatan)</td>
    			</tr>
			    <?php $no=1; foreach ($kel3 as $k3): ?>
			    <?php $kkm = $this->rm->kkm($id_kelas, $k3, $this->tahunAktif['id_tahun']) ?>
				<?php if ($no==6): ?>
					<tr>
					    <?php $kkm = $this->rm->kkm($id_kelas, 4, $this->tahunAktif['id_tahun']) ?>
						<td>6</td>
						<td><?= $dkn[$id_santri]['nilai'][4]['nama_mapel'].' (P)' ?><br><p class="kkm">KKM:<?=$kkm ?></p></td>
						<td><?= $dkn[$id_santri]['nilai'][4]['p'] ?></td>
						<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][4]['p'], 75)[0] ?></td>
						<td class="teks isi-deskrip"><?= $CI->tampilPredikat($id_santri, $id_kelas, 4, 'p') ?></td>

						<td><?= $dkn[$id_santri]['nilai'][4]['k'] ?></td>
						<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][4]['k'], 75)[0] ?></td>
						<td class="teks isi-deskrip"><?= $CI->tampilPredikat($id_santri, $id_kelas, 4, 'k') ?></td>
					</tr>
				<?php endif ?>
				<tr>
					<td><?=$no++?></td>
					...
					<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][$k3]['p'], 75)[0] ?></td>
					<td class="teks isi-deskrip"><?= $CI->tampilPredikat($id_santri, $id_kelas, $k3, 'p') ?></td>

					<td><?= $dkn[$id_santri]['nilai'][$k3]['k'] ?></td>
					<td><?= $CI->hitungPredikat($dkn[$id_santri]['nilai'][$k3]['k'], 75)[0] ?></td>
					<td class="teks isi-deskrip"><?= $CI->tampilPredikat($id_santri, $id_kelas, $k3, 'k') ?></td>
				</tr>
			    <?php endforeach ?>
			</tbody>
		</table>
